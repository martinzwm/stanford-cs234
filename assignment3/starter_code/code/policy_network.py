...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.observation_placeholder = tf.placeholder(dtype=tf.float32, shape=[None,self.observation_dim], name='obvervation')
    if self.discrete:
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      ...
      self.sampled_action = tf.reshape(tf.multinomial(action_logits, 1), [-1,]) # may not be the max softmax
      # Maximizing the logits = minimizing the negative log prob as calculated below
  ...
  def add_loss_op(self):
    ...
    loss = self.logprob * self.advantage_placeholder
